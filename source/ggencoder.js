...
$(function() {
    ...
    function decodeGBGG(code) {
        ...
        rawCode.address = (hex[2] << 8) | (hex[3] << 4) | hex[4] | 
            ((~hex[5] & 0xF) << 12);
    ...
    function decodeGenesis(code) {
        ...
        if (isValidGenesisCode(code)) {
            ...
            rawCode.value = ((hex[5] & 0x01) << 15) | 
                (((hex[6] & 0x18) >> 3) << 13) | ((hex[4] & 0x01) << 12) |
                (((hex[5] & 0x1E) >> 1) << 8) | (hex[0] << 3) | 
                ((hex[1] & 0x1C) >> 2);
                
            rawCode.address = hex[7] | ((hex[6] & 0x03) << 5) | 
                (((hex[3] & 0x10) >> 4) << 8) | (hex[2] << 9) |
                ((hex[1] & 0x03) << 14) | (((hex[4] & 0x1E) >> 1) << 16) |
                ((hex[3] & 0x0F) << 20);
        }
    ...
    function formatRawCode(code) {
        ...
        if (code.address != 0) {
            var length = ((system == SYSTEM_SUPERNINTENDO) || 
                          (system == SYSTEM_GENESIS)) ? 6 : 4;
    ...
    // handler called when the game genie code input box is changed
    function onChangeCode() {
        var code = $("#code").val();
        var rawCode;
        
        if ((system == SYSTEM_NINTENDO) && isValidNESCode(code)) {
            rawCode = decodeNES(code);
        } else if ((system == SYSTEM_SUPERNINTENDO) && isValidSNESCode(code)) {
            rawCode = decodeSNES(code);
        } else if ((system == SYSTEM_GENESIS) && isValidGenesisCode(code)) {
            rawCode = decodeGenesis(code);
        } else if ((system == SYSTEM_GBGG) && isValidGBGGCode(code)) {
            rawCode = decodeGBGG(code);
        }
        
        var formatted = formatRawCode(rawCode);
        
        $("#value").val(formatted.value);
        $("#address").val(formatted.address);
        $("#compare").val(formatted.compare);
    }
    
    // handler called when any of the raw code elements are changed
    function onChangeRawCode(event) {
        var rawCode = new RawCode();
        var code = "";
        
        rawCode.value = $("#value").val();
        rawCode.address = $("#address").val();
        rawCode.compare = $("#compare").val();
        
        if (isValidRawCode(rawCode)) {
            if (system == SYSTEM_NINTENDO) {
                code = encodeNES(rawCode);
            } else if (system == SYSTEM_SUPERNINTENDO) {
                code = encodeSNES(rawCode);
            } else if (system == SYSTEM_GENESIS) {
                code = encodeGenesis(rawCode);
            } else {
                code = encodeGBGG(rawCode);
            }
        }
    ...
    // handler for the system radio buttons
    function onChangeSystem(event) {
        if (event.target.value == "nes") {
            system = SYSTEM_NINTENDO;
        } else if (event.target.value == "snes") {
            system = SYSTEM_SUPERNINTENDO;
        } else if (event.target.value == "genesis") {
    ...
    // resets the form values and enables/disables the compare control
    function resetForm() {
        ...
        // compare only has meaning on Nintendo/Game Boy/Game Gear systems
        if ((system == SYSTEM_NINTENDO) || (system == SYSTEM_GBGG)) {
    ...
    // display the hidden application controls
    $(".ggencoder").show();
    
    // add handlers for the system radio button
    $(".systemList input").change(onChangeSystem);
    $(".rawCodeControls input").change(onChangeRawCode);
    $(".codeControls input").change(onChangeCode);
});
